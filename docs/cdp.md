...
`cdp` is a command-line tool for driving Chrome using the Chrome DevTools Protocol (CDP). It provides a REPL (Read-Eval-Print Loop) interface to execute CDP commands directly, which is useful for browser automation, debugging, and exploring the CDP API.
...
cdp --remote-host localhost --remote-port 9222
...
- `throttle <down> <up> <latency>` - Emulate network conditions
...
#### Coverage
- JavaScript coverage: `coverage_start`, `coverage_take`, `coverage_stop`

### Help Commands

- `help` - Show general help
- `help aliases` - Show all command aliases
- `help enhanced` - List enhanced commands (remote Chrome only)
...
See `skills/writing-cdp-scripts/references/script-format.md` for the script-format reference.
...
To launch against a copy of an existing Chrome profile (cookies, saved logins, preferences, history, and site storage; extensions are not copied):
