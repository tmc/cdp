...
| 5 | Delete internal/secureio | S→M | **DONE** | Deleted. Inlined `os.MkdirTemp("", "chrome-to-har-")` and `os.RemoveAll` at the 2 callsites in `internal/browserprofile/profile.go`; replaced `secureio.SecureDirPerms` with `0o700` literal at the 3 mkdir callsites. The 14 public symbols on secureio were dead weight: `CreateSecureTempDir` reinvented `os.MkdirTemp`, `SecureRemoveAll` wrapped `os.RemoveAll` with a meaningless post-stat verification, etc. -439 / +0 lines from the package; -519 lines from its tests. |
