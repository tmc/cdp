...
//
// Cancelling the returned context closes an owned tab or browser. WithExistingTarget
// preserves the attached target while detaching the session. To cancel a context
// while checking for errors, see [Cancel].
//
...
func NewContext(parent context.Context, opts ...ContextOption) (context.Context, context.CancelFunc) {
	...
	if !c.borrowed && c.first {
		if c.createBrowserContextParams != nil {
			panic("WithNewBrowserContext can not be used before Browser is initialized")
		}
		if c.existingBrowserContext {
			panic("WithExistingBrowserContext can not be used before Browser is initialized")
		}
	}
	if c.borrowed {
		if c.createBrowserContextParams != nil || c.existingBrowserContext {
			c.setupErr = errors.New("existing target cannot specify a browser context")
		}
	} else if c.createBrowserContextParams != nil && c.BrowserContextID != "" {
		panic("WithExistingBrowserContext can not be used when WithNewBrowserContext is specified")
	}
	if c.borrowed {
		// A borrowed target uses its existing browser context.
	} else if c.targetID == "" {
	...
	go func() {
		...
		if id := c.Target.TargetID; id != "" && !c.borrowed {
...
func WithTargetID(id target.ID) ContextOption {
	return func(c *Context) {
		if c.borrowed {
			c.setupErr = errors.New("conflicting target options")
		}
		c.targetSet = true
		c.targetID = id
	}
...
// WithExistingTarget attaches to id without taking ownership of the target.
// Canceling the context detaches its session but does not close the target.
// An allocator that owns the browser may still close the browser itself.
// Run returns an error if id is empty or this option is combined with another
// target option, WithNewBrowserContext, or WithExistingBrowserContext.
func WithExistingTarget(id target.ID) ContextOption {
	return func(c *Context) {
		if c.targetSet {
			c.setupErr = errors.New("conflicting target options")
		}
		...
		c.targetID = id
		if id == "" {
			c.setupErr = errors.New("existing target ID is empty")
		}
	}
}
